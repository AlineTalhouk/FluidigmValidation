#' Title unit test function for FluidigmValiation::labelMutationGroup
#'
#' @return
#' @export
#'
#' @examples
test.labelMutationGroup<-function(){
  ...
  checkEquals(sum(!normalDf.labelled$mutationGroup==normalDf.group.expected),0)
  ...
  checkEquals(sum(!attentionDf.labelled$mutationGroup==attentionDf.group.expected),0)

   wrongDf<-normalDf
   wrongDf$TumorType[7]<-"T3"
   checkException(labelMutationGroup(wrongDf))

   checkException(labelMutationGroup(subset(normalDf,select=-POS)))
   checkException(labelMutationGroup(subset(normalDf,select=-TumorType)))
}
